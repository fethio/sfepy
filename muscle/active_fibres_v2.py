...
materials = {
    'solid' : ({
        ...
        'K'  : vf_matrix * 1e9, # bulk modulus, solid.K yazılarak çağrılır. MPa
        'mu' : vf_matrix * 20e6, # shear modulus of neoHookean term, solid.mu yazılarak çağrılır. MPa
    },),
...
.  Notice that; if we write {'u.0' : 0.0}, horizontal(x) displacements of the nodes are prevented.
                ...
                if we write {'u.[1,2]' : 0.0}), x and y displacements of nodes are prevented.
"""
ebcs = {
    'l' : ('Left', {'u.all' : 0.0}),
    'r' : ('Right', {'u.0': 0.0006} ), 
...
def stress_strain(out, problem, state, extend=False):
    ...
    ev = problem.evaluate # problem.evaluate fonksiyonunu kısaltmak için 'ev değişkenine atanmış. Term name prefix 'ev' değildir.
                          # evaluate(....) fonksiyonu sonuç olarak array döndürür. Yani aşağıdaki strain ve stress değişkenleri array'dir.
...
solver_2 = {
    ...
    't1'    : 1e-2,
    'dt'    : None,
    'n_step' : 3, # has precedence over dt!
}
